...
export default function InputRadio({ hasLabel = true, id, name, label, onChange, value, checked }) {
	return hasLabel 
		? <Button>
			<label className="flex gap-2 items-center group">
				<InputRadioComponent id={ id } name={ name } label={ label } onChange={ onChange } value={ value } checked={ checked } />
			</label>
		</Button>
		: <InputRadioComponent id={ id } name={ name } label={ label } onChange={ onChange } value={ value } checked={ checked } />
}

function InputRadioComponent({ id, name, label, onChange, value, checked }) {
	return (
		<>
